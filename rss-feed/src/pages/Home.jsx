...
const Home = ({ articles }) => {
  // Definimos el estado 'query' para almacenar el valor de búsqueda del usuario.
    ...
    <div className="container mx-auto px-8">
      <form className="m-8">
        <div className="relative">
          ...
          {/* Input de búsqueda */}
          <input type="search" placeholder="Buscar título del artículo..."
            onChange={(e) => setQuery(e.target.value)}
            className="w-full py-3 pl-12 pr-4 text-gray-500 border rounded-md outline-none
             bg-gray-50 focus:bg-white focus:border-indigo-600"
          />
        </div>
      </form>
      {/* Filtramos y mostramos artículos basados en la consulta de búsqueda, por defecto salen todos*/}
      {articles.length > 0 && articles
      .filter(article => article.item.title.toLowerCase().includes(query))
      .map(article =>
      // Renderizamos cada artículo como un enlace a la página de detalle del artículo.
        <div key={article.item.id} className="m-8">
          <Link to={`/article/${article.item.id}`}
            ...
            <div className="h-full w-96 object-cover"
              dangerouslySetInnerHTML={{__html: getImageFromArticle(article.item.content)}}></div>
